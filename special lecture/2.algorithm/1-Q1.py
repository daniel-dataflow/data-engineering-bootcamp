    ...
    for i in range(ARRAY_LENGTH) :
        ...
        for k in range(ARRAY_LENGTH) :
            ...
            # 값이 100보다 큰 경우
                ...
                retData[i].append(numData[i][k] % 100)
            # 정상인 경우
            else :
                retData[i].append(numData[i][k])
...
def getMaxSum(numData): # 요구 사항에 맞춰 처리된 2차원 정수 배열
    ...
    ###########   여기부터 코딩 (2) ---------------->
    # 2x2 크기의 합 구하기
    # 5개의 2개씩 짝지어서 합을 구할 경우는 총 4번을 반복하면 된다.
    for i in range(ARRAY_LENGTH-1) :
        for k in range(ARRAY_LENGTH-1) :
            sum = 0
            # 각각의 포지션의 위치를 찍어서 합을 구한다.
            sum = numData[i][k] + numData[i][k+1] + numData[i+1][k] + numData[i+1][k+1]
            # 현재의 합이 최대합보다 클경우 교체한다.
            if sum > maxSum :
                maxSum = sum
